...
void sampler_run(i2c_master_bus_handle_t bus, esp_event_loop_handle_t event_loop, uint64_t sample_time_ms)
{
    esp_err_t err = shtc3_init(&sensor, bus, 0x70);
    if (err != ESP_OK)
    {
        ESP_LOGE(TAG, "SHTC3 init failed (%d)", err);
        return;
    }
